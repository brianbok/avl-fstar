...
let almost_b_inv t = 
  match t with
  | E -> True
  | T _ t1 _ t2 _ -> (abs(bf t) <= 2) /\ b_inv t1 /\ b_inv t2


val rebalance_fixes_almost_b_inv : t:avltree_ok ->
                                     Lemma (requires (almost_b_inv t))
                                           (ensures (b_inv (rebalance t)))
                                           [SMTPat (b_inv (rebalance t))]
let rebalance_fixes_almost_b_inv t = ()
...
(* Rebalance preserves k_inv *)
val rebalance_preserves_k_inv : r:avltree_ok ->
                                     Lemma (requires (k_inv r))
                                           (ensures (k_inv (rebalance r)))
                                           [SMTPat (k_inv (rebalance r))]
let rebalance_preserves_k_inv r = () 

val insert_preserves_k_inv : n:int ->
    				  t1:avl ->
				  t2:avl ->
   				  Lemma (requires
				  	(all (fun n' -> n' < n) t1 /\
					 all (fun n' -> n' > n) t2))
                         (ensures (k_inv (create_tree n (t1) t2)))
                         [SMTPat (k_inv (create_tree n (t1) t2))]
let insert_preserves_k_inv n t1 t2 = ()

val insert_avl: x:int -> t:avl -> 
  Tot (r:avltree_ok{ins_inv x t r /\ k_inv r}) 
